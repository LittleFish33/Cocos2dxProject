...
#pragma region 添加定时器
	...
	schedule(schedule_selector(OnlineGameScene::hit), 0.0f, kRepeatForever, 0); /* 被打判断 */
...
#pragma region 创建倒计时
	totalTime = 122;
	schedule(schedule_selector(OnlineGameScene::updateCountDown), 1);
...
void OnlineGameScene::onKeyPressed(EventKeyboard::KeyCode code, Event* event)
{
	...
	if (bothCanmove) {
		...
		case cocos2d::EventKeyboard::KeyCode::KEY_S:
			...
			player2->defend();
			S_KeyState = true;
			data.codeSend = KEYCODESEND::DOWN;
			client->sendMessage((char*)(&data), sizeof(SocketData));
			break;
...
void OnlineGameScene::onKeyReleased(EventKeyboard::KeyCode code, Event* event)
{
	...
	if (bothCanmove) {
		...
		case cocos2d::EventKeyboard::KeyCode::KEY_S:
			...
			yIsBreak = false;
			S_KeyState = false;
			data.codeSend = KEYCODESEND::DOWNRELEASE;
			client->sendMessage((char*)(&data), sizeof(SocketData));
			break;
		case cocos2d::EventKeyboard::KeyCode::KEY_G:
			yIsBreak = false;
...
void OnlineGameScene::updateCountDown(float delay)
{
	Vec2 origin = Director::getInstance()->getVisibleOrigin();
	if (totalTime == 122) {
...
#pragma endregion
	...
	/*弹出游戏结束界面,  需要延迟1s*/
	scheduleOnce(schedule_selector(OnlineGameScene::CallGameOverScene), delay);
}
...
void OnlineGameScene::CallGameOverScene(float dt)
{
...
void OnlineGameScene::explosion()
{
